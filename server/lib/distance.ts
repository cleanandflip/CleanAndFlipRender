...
export function /* SSOT-FORBIDDEN \bisLocalMiles\( */ isLocalMiles(lat: number | null, lng: number | null): boolean {
  if (lat == null || lng == null) return false;
  return milesBetween({lat, lng}, WAREHOUSE) <= LOCAL_RADIUS_MILES;
}

export function isLocalAddress(address: any): boolean {
  return /* SSOT-FORBIDDEN \bisLocalMiles\( */ isLocalMiles(address.lat, address.lng);
}
...
export function setupAuth(app: Express) {
	// Initialize Google OAuth strategy only if configured
	if (process.env.GOOGLE_CLIENT_ID && process.env.GOOGLE_CLIENT_SECRET) {
		initializeGoogleAuth();
	} else {
		Logger.warn('Google OAuth not configured; skipping strategy init');
	}
	
	// If no DB URL, fall back to in-memory session for slate boot
	const hasDb = !!ENV.devDbUrl;
	...
	if (hasDb) {
		try {
			...
			Logger.error('[SESSION] PostgreSQL store init failed, using MemoryStore:', error.message);
		}
	} else {
		Logger.warn('[SESSION] No DATABASE_URL; using MemoryStore for slate boot');
	}
	...
	if (appliedStore && appliedStore.constructor.name === 'PGStore') {
		...
		// In slate mode, allow MemoryStore with warnings to enable boot
	}
			...
			new GoogleStrategy(
				{
					...
					callbackURL: process.env.NODE_ENV === 'production'
						? `${ENV.frontendOrigin}/api/auth/google/callback`

import { APP_ENV, ENV } from '../config/env';

export async function verifyProductSchema() {
	// Skip schema verification entirely if no database is configured
	if (!ENV.devDbUrl) {
		console.warn('[SCHEMA] Skipping schema verification (no DATABASE_URL configured)');
		return;
	}

	// Import pool lazily to avoid creating a connection when not needed
	const { universalPool } = await import('../db/universal-pool');

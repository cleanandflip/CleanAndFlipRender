...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Unified search suggestions endpoint
    ...
    try {
      ...
      const [productResults, categoryResults] = await Promise.all([
        ...
        db.select({
          ...
          count: sql<number>`(SELECT COUNT(*) FROM products WHERE category_id = ${categories.id} AND active = true)`,
  ...
  app.get('/api/navigation/menu', apiLimiter, async (req, res) => {
    try {
      ...
      res.json({
        ...
        quickLinks: [
          ...
          { label: 'Local Pickup', url: '/products?local=true' }
        ]
      });
    } catch (error) {
      Logger.error('Navigation menu error:', error);

...
PostgreSQL is the primary database. The system is transitioning from external Neon databases (lucky-poetry/muddy-moon) to a unified Replit-managed PostgreSQL database for both development and production environments. This eliminates external dependencies and provides integrated database management. Environment classification maintains safety guards while using a single database instance. Drizzle Kit manages schema migrations. PostgreSQL's `tsvector` is used for full-text search, and session storage is database-backed. Key tables include users, products, categories, orders, cart items, and equipment submissions with 21 total tables in the complete schema.
